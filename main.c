...
int array[MAX_SIZE]; // D�clarer le tableau comme un tableau statique
int size = 0;
...
while (!WindowShouldClose()) {
    ...
    if (IsKeyPressed(KEY_ONE)) { // Utiliser IsKeyPressed de raylib
        if (size < MAX_SIZE) {
            ...
            InsertFirst(array, &size, value); // Ajouter un nombre au d�but
        }
    } else if (IsKeyPressed(KEY_TWO)) {

        if (size < MAX_SIZE) { // Ajouter cette condition
            int value = GetRandomValue(10, 200); // Utiliser GetRandomValue de raylib
            InsertLast(array, &size, value); // Ajouter un nombre � la fin
        }
    } else if (IsKeyPressed(KEY_THREE)) {
        if (size > 0) {
            DeleteFirst(array,  &size); // Supprimer le premier �l�ment
        }
    } else if (IsKeyPressed(KEY_FOUR)) {
        if (size > 0) {
            DeleteLast(array, &size); // Supprimer le dernier �l�ment
        }
    } else
    if (!isSorting) {
        ...
        if (size > 0) { // Ajouter cette condition
            ...
            if (index != -1) {
                DrawText(TextFormat("L'element %d se trouve � l'index %d", value, index), 10, 320, 20, DARKGRAY);
...
void QuickSort(int *array, int low, int high) {
    if (low < high) {
        ...
        while (i < j) {
            ...
            if (i < j) {
                // �changer array[i] et array[j]
                Swap(array, i, j);

                // Mettre � jour l'index en surbrillance
                BeginDrawing();
                ClearBackground(RAYWHITE);
                ...
                // Attendre un court instant pour voir le changement
                startTime = GetTime(); // Enregistrer le temps de d�part
                while (GetTime() - startTime < 2) {} // Attendre 2 seconde
            }
        }

        // �changer array[low] et array[j]
        Swap(array, low, j);
        ...
        // Afficher le tableau apr�s la partition
        BeginDrawing();
        ClearBackground(RAYWHITE);
        ...
        // Trier les sous-tableaux � gauche et � droite du pivot
...
int BinarySearch(int *array, int low, int high, int value) {
    ...
    int mid = (low + high) / 2;
    // Si la valeur du milieu est �gale � la valeur recherch�e, on la renvoie
    if (array[mid] == value) {
        return mid;
    }
    // Sinon, si la valeur du milieu est plus grande que la valeur recherch�e, on cherche dans la partie gauche du tableau
...
// Fonction qui �change deux �l�ments du tableau
void Swap(int *array, int i, int j) {
    // Stocker la valeur du tableau � l'index i dans une variable temporaire
    int temp = array[i];
    // Affecter la valeur du tableau � l'index j au tableau � l'index i
    array[i] = array[j];
    // Affecter la valeur de la variable temporaire au tableau � l'index j
    array[j] = temp;
}


// Fonction qui supprime le premier �l�ment du tableau
void DeleteFirst(int *array, int *size) {
    // Si la taille du tableau est sup�rieure � 0
    if (*size > 0) {
        // Parcourir le tableau avec une boucle for
        for (int i = 0; i < *size - 1; i++) {
            // Affecter la valeur du tableau � l'index i+1 au tableau � l'index i
            array[i] = array[i + 1];
        }
...
// Fonction qui supprime le dernier �l�ment du tableau
void DeleteLast(int *array, int *size) {
    // Si la taille du tableau est sup�rieure � 0
    if (*size > 0) {
        // Diminuer la taille du tableau de 1
...
// Fonction qui ajoute un �l�ment au d�but du tableau
void InsertFirst(int *array, int *size, int value) {
    // Si la taille du tableau est inf�rieure � la taille maximale
    if (*size < MAX_SIZE) {
        // Parcourir le tableau à l'envers avec une boucle for
        for (int i = *size - 1; i >= 0; i--) {
            // Affecter la valeur du tableau � l'index i au tableau � l'index i+1
            array[i + 1] = array[i];
        }
        // Affecter la valeur � ajouter au tableau à l'index 0
        array[0] = value;
...
// Fonction qui ajoute un �l�ment � la fin du tableau
void InsertLast(int *array, int *size, int value) {
    // Si la taille du tableau est inferieure à la taille maximale
    if (*size < MAX_SIZE) {
        // Affecter la valeur à ajouter au tableau a l'index de la taille actuelle
        array[*size] = value;
        // Augmenter la taille du tableau de 1
        (*size)++;
    }
